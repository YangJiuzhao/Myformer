...
class Exp_former(Exp_Basic):
    ...
    def _build_model(self):        
        if self.args.model == 'former':
            model = former(self.args.data_dim, self.args.in_len, self.args.out_len, self.args.seg_lens, 
                    self.args.d_model, self.args.d_ff, self.args.n_heads, self.args.a_layers, 
                    self.args.dropout, self.device).float()
        elif self.args.model == 'iformer':
            model = iformer(self.args.data_dim, self.args.in_len, self.args.out_len, self.args.seg_lens, 
                    self.args.d_model, self.args.d_ff, self.args.n_heads, self.args.a_layers, 
                    self.args.dropout, self.device).float()
